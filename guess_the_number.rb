...
def guess_with_loop_do(number_to_guess, guess_number, range)
  loop do
    puts "Enter your guessing number between #{range[0]} and #{range[1]}"
    guess_number = gets.chomp.to_i
    break if guess_number == number_to_guess
...
def guess_with_unless(number_to_guess, guess_number, range)
  loop do
    puts "Enter your guessing number between #{range[0]} and #{range[1]}"
    guess_number = gets.chomp.to_i
    unless number_to_guess == guess_number
      puts guess_number > number_to_guess ? "Your number is High" : "Your number is Low"
    else
      break
    end
...
puts "guessing using loop do"
guess_with_loop_do(number_to_guess, guessed_number, guessing_range)

puts "guessing using unless"
guess_with_unless(number_to_guess, guessed_number, guessing_range)